...
func KeepalivedEndpoint(c echo.Context) error {
	// 访问数据库测试是否成功
	_, err := identityConfigRepository.FindPasswordConfig()
	if err != nil {
		return Fail(c, -1, "")
	}
...
func HaClusterUpdateEndpoint(c echo.Context) error {
	...
	if haConfig.Enabled == "true" {
		...
		if err != nil {
			...
			_ = FileSyncAndDisable()
			_ = ServiceMonitorDisable()
			return FailWithDataOperate(c, 500, "集群服务已开启,写入失败", "", err)
		}
...
func StartMasterCluster(masterIp, StandbyDeviceIP string) (err error) {
	...
	_, err = utils.ExecShell("firewall-cmd --zone=public --add-port=4444/tcp --permanent")
	if err != nil {
		log.Errorf("开放4444端口: %v", err)
	}
...
func CloseMariaDBSync() error {
	...
	_, err = utils.ExecShell("firewall-cmd --zone=public --remove-port=4444/tcp --permanent")
	if err != nil {
		log.Errorf("关闭4444端口错误: %v", err)
	}
...
`
	...
	_, _ = utils.ExecShell("nohup sh /tkbastion/tools/scripts/inotify.sh  > /dev/null  2>&1 &")
	// 检查运行是否成功
...
`
	...
	if err != nil {
		log.Errorf("ExecShell firewall-cmd --reload error: %v", err)
	}
	// 检查keepalived是否启动
...
func ServiceMonitorDisable() (err error) {
	...
	// 关闭keepalived的端口
	_, _ = utils.ExecShell("firewall-cmd --zone=public --remove-port=112/tcp --permanent")
	return nil
}
...
`
	// 2. 写入配置文件
	if err := utils.WriteFileDirect(filePath, mysqld); err != nil {
		return err
	}
	return nil
}
