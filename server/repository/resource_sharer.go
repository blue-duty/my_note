...
func (r *ResourceSharerRepository) Create(resourceSharer *model.ResourceSharer) error {
	return r.DB.Create(resourceSharer).Error
}
...
func (r *ResourceSharerRepository) FindByResourceIdAndUserId(assetId, userId string) (resourceSharers []model.ResourceSharer, err error) {
	// 查询其他用户授权给该用户的资产
	groupIds, err := userGroupMemberRepository.FindUserGroupIdsByUserId(userId)
	if err != nil {
		return
	}
...
func (r *ResourceSharerRepository) FindByResourceIdAndUserIdTime(assetId, userId string) (resourceSharers []model.ResourceSharer, err error) {
	// 查询其他用户授权给该用户的资产
	groupIds, err := userGroupMemberRepository.FindUserGroupIdsByUserId(userId)
	if err != nil {
		return
	}
	db := r.DB.Where("( resource_id = ? and user_id = ? and end_time is not null )", assetId, userId)
	if len(groupIds) > 0 {
		db = db.Or("user_group_id in ?", groupIds)
	}
	err = db.Find(&resourceSharers).Error
	...
	return r.DB.Transaction(func(tx *gorm.DB) (err error) {
		...
		for i := range resourceIds {
			...
			case "credential":
				...
				owner = resource.Owner
			case "assetgroup":
				resource := model.AssetGroup{}
				if err = tx.Where("id = ?", resourceId).First(&resource).Error; err != nil {
					return errors.Wrap(err, "find assetgroup  fail")
				}
				owner = resource.Owner
			}
...
func (r *ResourceSharerRepository) Truncate() error {
	return r.DB.Exec("TRUNCATE TABLE resource_sharers").Error
}
